  ...
  try {
    ...
    if (token) {
      ...
      return res.status(200).json({
        ...
        data: {
          token,
          user: { email, subscription: user.subscription, avatar: user.avatar },
        },
      });
    }
    next({
      status: 401,
      message: "Email or password may be wrong. Also verificate account",
    });
  } catch (e) {
    next(e);
  }
...
const avatars = async (req, res, next) => {
  ...
  return res.status(200).json({ status: "success", code: 200, avatarUrl: url });
};
